            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {diagrams.map(d => (
                <div key={d.id} style={{ background: "#1E2533", border: "1px solid #324055", borderRadius: "14px", padding: "16px" }}>
                  <p style={{ fontWeight: 700, color: "#F8FAFF", fontSize: "15px", marginBottom: d.description ? "4px" : "12px", fontFamily: "'DM Sans',sans-serif" }}>{d.title}</p>
                  {d.description && <p style={{ fontSize: "13px", color: "#7E8798", marginBottom: "12px", fontFamily: "'DM Sans',sans-serif" }}>{d.description}</p>}
                  {d.content?.blocks && (
                    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                      {d.content.blocks.map((b, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "8px" }}>
                          <div style={{ width: "22px", height: "22px", borderRadius: "7px", background: "rgba(123,97,255,0.15)", border: "1px solid rgba(123,97,255,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: "12px", color: "#A78BFA", fontWeight: 700, marginTop: "1px" }}>{i + 1}</div>
                          <div>
                            <p style={{ fontSize: "14px", fontWeight: 600, color: "#F8FAFF", fontFamily: "'DM Sans',sans-serif" }}>{b.label}</p>
                            {b.note && <p style={{ fontSize: "13px", color: "#7E8798", fontFamily: "'DM Sans',sans-serif" }}>{b.note}</p>}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                return (
                  <div key={v.id} style={{ background: "#1E2533", border: "1px solid #324055", borderRadius: "14px", overflow: "hidden" }}>
                    ...
                    <div style={{ padding: "12px 14px" }}>
                      <p style={{ fontWeight: 700, color: "#F8FAFF", fontSize: "15px", fontFamily: "'DM Sans',sans-serif" }}>{v.title}</p>
                      {v.description && <p style={{ fontSize: "13px", color: "#7E8798", marginTop: "3px", fontFamily: "'DM Sans',sans-serif" }}>{v.description}</p>}
                    </div>
                  </div>
                );
              })}
              ...
              {articles.map(a => (
                <a key={a.id} href={a.url ?? "#"} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
                  <div style={{ background: "#1E2533", border: "1px solid #324055", borderRadius: "12px", padding: "12px 14px" }}>
                    <p style={{ fontWeight: 700, color: "#F8FAFF", fontSize: "15px", fontFamily: "'DM Sans',sans-serif" }}>{a.title}</p>
                    {a.description && <p style={{ fontSize: "13px", color: "#7E8798", marginTop: "3px", fontFamily: "'DM Sans',sans-serif" }}>{a.description}</p>}
                    <p style={{ fontSize: "12px", color: "#468BFF", marginTop: "4px", fontFamily: "'DM Sans',sans-serif" }}>{a.url}</p>
                  </div>
                </a>
              ))}
            </div>
