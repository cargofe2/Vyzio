...
const T = {
  badge: { es: "La academia para la era de la IA", en: "The academy for the AI era" },
  ...
  featuresH2: { es: "Por qué Bymyzai", en: "Why Bymyzai" },
  features: {
    es: [
  ...
  ctaFinalH2: { es: "Tu futuro empieza hoy.", en: "Your future starts today." },
  ctaFinalSub: { es: "Gratis. Sin tarjeta. Sin excusas.", en: "Free. No card. No excuses." },
  navSignIn: { es: "Entrar", en: "Sign in" },
  footerLinks: {
    es: [["Pricing", "/pricing"], ["Términos", "/terms"], ["Privacidad", "/privacy"], ["Disclaimer", "/disclaimer"]],
    en: [["Pricing", "/pricing"], ["Terms", "/terms"], ["Privacy", "/privacy"], ["Disclaimer", "/disclaimer"]],
  },
  zaiGreeting: { es: "¡Hola! ¿Listo para explorar cómo funciona la IA? →", en: "Hey! Ready to explore how AI works? →" },
  zaiLabel: { es: "ZAI · Tu tutor IA", en: "ZAI · Your AI tutor" },
};

const FEATURE_ICONS = [
  <svg key="f1" width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="8" r="4" stroke="#7B61FF" strokeWidth="1.8"/><path d="M5 20C5 17.2 8.1 15 12 15C15.9 15 19 17.2 19 20" stroke="#7B61FF" strokeWidth="1.8" strokeLinecap="round"/><path d="M17 4.5C18.2 5.1 19 6.4 19 8" stroke="#7B61FF" strokeWidth="1.8" strokeLinecap="round"/><circle cx="19" cy="3.5" r="1" fill="#7B61FF"/></svg>,
  <svg key="f2" width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="2" y="6" width="20" height="13" rx="3" stroke="#F2C04D" strokeWidth="1.8"/><path d="M6 12.5H10M8 10.5V14.5" stroke="#F2C04D" strokeWidth="1.8" strokeLinecap="round"/><circle cx="16" cy="12.5" r="1.2" fill="#F2C04D"/><circle cx="19" cy="12.5" r="1.2" fill="#F2C04D"/></svg>,
  <svg key="f3" width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M12 2L15 8H21L16.5 12.5L18.5 19L12 15.5L5.5 19L7.5 12.5L3 8H9L12 2Z" stroke="#36D399" strokeWidth="1.8" strokeLinejoin="round"/></svg>,
  <svg key="f4" width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="#468BFF" strokeWidth="1.8"/><path d="M2 12H22" stroke="#468BFF" strokeWidth="1.8"/><path d="M12 2C9.5 5.5 8 8.5 8 12C8 15.5 9.5 18.5 12 22" stroke="#468BFF" strokeWidth="1.8"/><path d="M12 2C14.5 5.5 16 8.5 16 12C16 15.5 14.5 18.5 12 22" stroke="#468BFF" strokeWidth="1.8"/></svg>,
  <svg key="f5" width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M12 3L4 7V12C4 16.4 7.6 20.5 12 21C16.4 20.5 20 16.4 20 12V7L12 3Z" stroke="#FF7DAE" strokeWidth="1.8" strokeLinejoin="round"/><path d="M9 12L11 14L15 10" stroke="#FF7DAE" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>,
  <svg key="f6" width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="3" y="11" width="18" height="10" rx="2" stroke="#26C6DA" strokeWidth="1.8"/><path d="M7 11V7C7 4.8 9.2 3 12 3C14.8 3 17 4.8 17 7V11" stroke="#26C6DA" strokeWidth="1.8" strokeLinecap="round"/><circle cx="12" cy="16" r="1.5" fill="#26C6DA"/></svg>,
];

const FEATURE_COLORS = ["#7B61FF", "#F2C04D", "#36D399", "#468BFF", "#FF7DAE", "#26C6DA"];
...
export default async function HomePage() {
  ...
  const lang: "en" | "es" = acceptLang.toLowerCase().startsWith("en") ? "en" : "es";

  const t = (key: keyof typeof T) => {
    const val = T[key];
    if (typeof val === "object" && "es" in val && "en" in val) {
      return (val as { es: string; en: string })[lang];
    }
    return "";
  };
    ...
    <main style={{ minHeight: "100vh", background: "#0F1420", color: "#F8FAFF", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section className="hero-layout" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "56px 24px", gap: "48px", minHeight: "calc(100vh - 77px)", textAlign: "center" }}>
        ...
        <div className="hero-text" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "6px 14px", borderRadius: "100px", background: "rgba(123,97,255,0.12)", border: "1px solid rgba(123,97,255,0.25)", marginBottom: "24px" }}>
            ...
            <span style={{ fontSize: "12px", color: "#A78BFA", fontWeight: 600, letterSpacing: "0.5px" }}>{t("badge")}</span>
          </div>
        ...
        <div className="hero-visual" style={{ display: "none", flexDirection: "column", gap: "12px", minWidth: "360px", maxWidth: "420px" }}>
          <div className="hero-visual-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
            {WORLDS.slice(0, 4).map(({ name, color, bg, border, icon }) => (
              <div key={name.es} style={{ borderRadius: "16px", padding: "16px", background: bg, border: `1px solid ${border}` }}>
                ...
                <p style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "12px", color, lineHeight: 1.3 }}>{name[lang]}</p>
              </div>
            ))}
          </div>
          {/* ZAI preview card */}
          <div style={{ borderRadius: "16px", padding: "16px 20px", background: "rgba(123,97,255,0.06)", border: "1px solid rgba(123,97,255,0.15)", display: "flex", alignItems: "center", gap: "14px" }}>
            ...
            <div>
              <p style={{ fontSize: "11px", color: "#A78BFA", fontWeight: 700, marginBottom: "4px" }}>{t("zaiLabel")}</p>
              <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", lineHeight: 1.5 }}>{t("zaiGreeting")}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section-worlds" style={{ padding: "56px 24px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <div className="worlds-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", maxWidth: "400px", margin: "0 auto" }}>
          {WORLDS.map(({ name, color, bg, border, icon }) => (
            <div key={name.es} style={{ borderRadius: "16px", padding: "14px", background: bg, border: `1px solid ${border}` }}>
              ...
              <p style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "11px", color, lineHeight: 1.3 }}>{name[lang]}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: "56px 24px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <div className="features-grid" style={{ display: "grid", gridTemplateColumns: "1fr", gap: "12px", maxWidth: "820px", margin: "0 auto" }}>
          {T.features[lang].map(({ title, desc }, i) => (
            <div key={title} style={{ borderRadius: "16px", padding: "20px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div style={{ width: "40px", height: "40px", borderRadius: "12px", background: `${FEATURE_COLORS[i]}15`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "12px" }}>
                {FEATURE_ICONS[i]}
              </div>
      ...
      <footer style={{ padding: "32px 24px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="footer-inner" style={{ display: "flex", flexDirection: "column", gap: "16px", alignItems: "center", maxWidth: "1100px", margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: "20px", flexWrap: "wrap", justifyContent: "center" }}>
            {(T.footerLinks[lang] as [string, string][]).map(([label, href]) => (
