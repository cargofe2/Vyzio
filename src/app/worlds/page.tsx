        ...
        <div style={{ padding: "18px 16px 4px" }}>
          <div style={{ background: "linear-gradient(160deg, #2A1F5C, #1A1440 60%, #0F1420)", border: "1px solid rgba(123,97,255,0.35)", borderRadius: "22px", padding: "18px", display: "flex", alignItems: "center", gap: "14px" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: "10px", color: "rgba(255,255,255,0.5)", fontWeight: 700, fontFamily: "'DM Sans',sans-serif", marginBottom: "2px" }}>{(lang === "en" ? {"level-1":"Level 0 · Origins","level-new-1":"Level 1 · Explorer","level-new-2":"Level 2 · Thinker","level-new-3":"Level 3 · Creator","level-new-4":"Level 4 · Builder","level-new-5":"Level 5 · Architect","level-new-6":"Level 6 · Founder","level-new-7":"Level 7 · Researcher","level-new-8":"Level 8 · Residency"} : LEVEL_NAMES)[selectedWorld?.levelId ?? levelId] ?? ""}</p>
              <p style={{ fontFamily: "'Syne',sans-serif", fontWeight: 900, color: "#F8FAFF", fontSize: "18px", marginBottom: "6px", wordBreak: "break-word" }}>{lang === "en" ? (selectedWorld?.name_en ?? selectedWorld?.name ?? "World") : (selectedWorld?.name ?? "Mundo")}</p>
          ...
          return (
            <Link key={w.id} href={`/worlds?id=${w.id}`} style={{ textDecoration: "none" }}>
              <div style={{ background: "#1E2533", border: "1px solid #324055", borderRadius: "18px", padding: "14px", position: "relative", overflow: "hidden" }}>
                ...
                <p style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "13px", color: "#F8FAFF", marginBottom: "8px", lineHeight: 1.3, wordBreak: "break-word" }}>{w.name}{w.isOptional && <span style={{ fontSize: "9px", fontWeight: 700, color: "#F2C04D", marginLeft: "6px", background: "rgba(242,192,77,0.12)", padding: "1px 6px", borderRadius: "6px", verticalAlign: "middle" }}>{lang === "en" ? "Opt." : "Opc."}</span>}</p>
