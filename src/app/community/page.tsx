        ...
        {items.map(({ href, label }) => {
          ...
          return (
            <Link key={href} href={href} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "3px", textDecoration: "none", padding: "4px 0" }}>
              <div style={{ width: "40px", height: "40px", background: isActive ? "rgba(123,97,255,0.2)" : "transparent", border: isActive ? "1px solid rgba(123,97,255,0.4)" : "1px solid transparent", borderRadius: "13px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px" }}>
                {href === "/dashboard" ? "🏠" : href === "/worlds" ? "🌍" : href === "/vy" ? "🤖" : href === "/community" ? "🏆" : "👤"}
              </div>
              <span style={{ fontSize: "8px", fontFamily: isActive ? "'Syne',sans-serif" : "'DM Sans',sans-serif", fontWeight: isActive ? 800 : 500, color: isActive ? "#7B61FF" : "#7E8798", letterSpacing: isActive ? "0.5px" : "0" }}>{isActive ? label.toUpperCase() : label}</span>
            </Link>
          );
        })}
      </nav>
