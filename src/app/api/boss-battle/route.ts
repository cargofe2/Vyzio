  ...
  try {
    ...
    if (!levelIsFree && plan === "STARTER") {
      return NextResponse.json({
        error: "plan_required",
        message: "Los Boss Battles de niveles avanzados requieren plan Pro.",
      }, { status: 403 });
    }
    ...
    const prompt = `Eres ZAI evaluando un Boss Battle en Bymyzai.
Proyecto: "${lesson.title}"
Entrega: "${submission.slice(0, 600)}"
Demuestra comprension real del tema? Responde exactamente:
[APROBADO] o [REINTENTAR]
Luego 2 frases de feedback directo. Maximo 60 palabras total.`;
    ...
    return NextResponse.json({ passed, feedback, attempts, attemptsUsed: todayAttempts + 1, attemptsLimit: limit });
  } catch (error) {
